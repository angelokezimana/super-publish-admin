...
<div class="container mb-2">
    <p class="h1 text-center">Liste des publications</p>
        ...
        <div class="card rounded shadow bg-white border-bottom-primary">
            <div class="mw-100">
                <a href="{{ route('publications.show', $publication) }}">
                    <img src="{{ asset('storage/images/'.$publication->photo) }}" alt="{{ $publication->title }}" class="w-100 rounded-top">
                </a>
            </div>
            ...
            <div class="card-footer">
                ...
                @can('Supprimer Evenements')
                    <form action="{{ route('publications.destroy', $publication) }}" method="post" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Voulez-vous vraiment supprimer cette entrée ?')">
